...
directions, in two colors checkered.  Afterwards the sign is captured and
each dot's rendered centre is compared with where the measured grid says that
texel is.  A dot a texel off shows as a residual of a whole pitch; a dot the
game never painted shows as missing.
...
def dot_plan(columns: int, rows: int, every: int, colors) -> PaintPlan:
    groups = []
    for parity, color in enumerate(colors):
        strokes = tuple(
            Stroke(x, y, x, y)
            for y in range(every // 2, rows, every)
            for x in range(every // 2, columns, every)
            if ((x // every) + (y // every)) % 2 == parity
        )
        groups.append(ColorGroup(color, strokes, 1))
...
def main() -> int:
    ...
    colors = ((30, 60, 220), (220, 40, 40))
    plan = dot_plan(args.columns, args.rows, args.every, colors)
    print(f"plan: {plan.width}x{plan.height}, {plan.stroke_count} dots", flush=True)
    ...
    result: dict = {"state": state.value, "grid": grid.to_dict() if grid else None}
