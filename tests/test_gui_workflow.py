...
def test_rust_settings_round_trip_and_command_preview(window: MainWindow) -> None:
    window.console_key_combo.setCurrentText("CTRL+F1")
    window.selected_brush_spin.setValue(1)
    window.brush_spacing_spin.setValue(0.25)
    ...
    preview = window.game_commands_label.text().splitlines()
    assert "paint.selectedbrush 1" in preview
    assert "paint.brushspacing 0.25" in preview
    assert "paint.brushopacity 1" in preview

    window._apply_settings(
        {**document, "game": {"console_key": "F1", "selected_brush": 3, "brush_spacing": 0.01}}
    )
    assert window.console_key_combo.currentText() == "F1"
    assert window.selected_brush_spin.value() == 3
    assert window.brush_spacing_spin.value() == pytest.approx(0.01)


def test_apply_in_rust_types_the_commands_through_the_console(
    window: MainWindow, monkeypatch: pytest.MonkeyPatch, qtbot
) -> None:
    import app.input_controller as input_module

    created: list[MockInputController] = []

    class RecordingDryRun(MockInputController):
        skip_timing = True
    ...
    monkeypatch.setattr(input_module, "DryRunInputController", RecordingDryRun)
    window.dry_run_check.setChecked(True)
    window.console_key_combo.setCurrentText("CTRL+F1")

    window.apply_game_settings_button.click()
    qtbot.waitUntil(lambda: created and not window._debug_running, timeout=5000)

    events = [(event.kind, event.value) for event in created[0].events]
    typed = [value for kind, value in events if kind == "type_text"]
    assert typed == list(window._game_convars().commands())
    assert events[0] == ("key_down", "CTRL")
    assert ("key_down", "F1") in events
    assert not created[0].held_keys
    assert window._debug_controller is None
